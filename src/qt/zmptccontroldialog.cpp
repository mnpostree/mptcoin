...
#include "ui_zmptccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zmptcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zmptcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zMPTC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zMPTC seed used to mint this zMPTC is not the same as currently hold in the wallet";
            else
