...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZMPTCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zmptcTracker->HasMintTx(hash))) {
            //zMPTC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZMPTC;
...
bool IsZMPTCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZMPTC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZMPTC || type == TransactionRecord::MNReward) {
