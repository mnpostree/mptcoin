...
public:
    ...
    bool WriteZMPTCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMPTCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMPTCSeed_deprecated(uint256& seed);
    bool EraseZMPTCSeed();
    bool EraseZMPTCSeed_deprecated();

    bool WriteZMPTCCount(const uint32_t& nCount);
    bool ReadZMPTCCount(uint32_t& nCount);
