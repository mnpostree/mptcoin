...
CzMPTCWallet::CzMPTCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmptc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMPTCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMPTCSeed_deprecated()) {
                    LogPrintf("%s: Updated zMPTC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmptc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmptc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMPTCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMPTCCount(nCountLastUsed);
    else if (!walletdb.ReadZMPTCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMPTCWallet::Lock()
{
    seedMaster = 0;
}

void CzMPTCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMPTCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMPTC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMPTCWallet::LoadMintPoolFromDB()
{
...
void CzMPTCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMPTCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMPTCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmptcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMPTCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMPTC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmptcTracker which also adds to database
    pwalletMain->zmptcTracker->Add(dMint, true);
...
void CzMPTCWallet::SeedToZMPTC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMPTCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMPTCWallet::GenerateDeterministicZMPTC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMPTCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMPTCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMPTCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
