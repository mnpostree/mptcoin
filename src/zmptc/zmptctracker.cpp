...
CzMPTCTracker::CzMPTCTracker(std::string strWalletFile)
{
...
CzMPTCTracker::~CzMPTCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMPTCTracker::Init()
{
...
bool CzMPTCTracker::Archive(CMintMeta& meta)
{
...
bool CzMPTCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMPTCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMPTCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMPTCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzMPTCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzMPTCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzMPTCTracker::GetSerialHashes()
{
...
CAmount CzMPTCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMPTCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMPTCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMPTCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMPTCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMPTCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMPTCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMPTCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMPTCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMPTCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMPTCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMPTCWallet* zMPTCWallet)
{
    bool iszMPTCWalletInitialized = (NULL != zMPTCWallet);
    CMintMeta meta;
    ...
    if (! iszMPTCWalletInitialized)
        zMPTCWallet = new CzMPTCWallet(strWalletFile);
    meta.isSeedCorrect = zMPTCWallet->CheckSeed(dMint);
    if (! iszMPTCWalletInitialized)
        delete zMPTCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMPTCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMPTCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMPTCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMPTCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMPTCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMPTCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMPTCWallet* zMPTCWallet = new CzMPTCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMPTCWallet);
        }
        delete zMPTCWallet;
        LogPrint("zero", "%s: added %d dzmptc from DB\n", __func__, listDeterministicDB.size());
    }
