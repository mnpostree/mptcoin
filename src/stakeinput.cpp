...
// The zMPTC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zMPTC is a hash of the serial
...
{
    //Create an output returning the zMPTC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMPTCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMPTC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMPTC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMPTCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMPTC output", __func__);
...
{
    CzMPTCTracker* zmptcTracker = pwallet->zmptcTracker.get();
    CMintMeta meta;
    if (!zmptcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmptcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a MPTC stake is the outpoint
